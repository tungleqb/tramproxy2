...
def get_profile(token):
    ...
    res = requests.get(f"{BASE_URL}/api/user/profile", headers=headers)
    print(res.status_code, res.json())
...
def update_user(token):
    ...
    res = requests.put(f"{BASE_URL}/api/user/update", headers=headers, json=data)
    print(res.status_code, res.json())

def list_proxies(token):
    print_section("List Proxies")
    headers = {"Authorization": f"Bearer {token}"}
    res = requests.get(f"{BASE_URL}/api/proxy/list", headers=headers)
    print(res.status_code, res.json())
...
def buy_proxy(token):
    ...
    res = requests.post(f"{BASE_URL}/api/proxy/buy", headers=headers, json=data)
    print(res.status_code, res.json())
...
def renew_proxy(token, proxy_id):
    ...
    res = requests.post(f"{BASE_URL}/api/proxy/renew", headers=headers, json=data)
    print(res.status_code, res.json())
...
def deposit(token):
    ...
    res = requests.post(f"{BASE_URL}/api/payment/deposit", headers=headers, json=data)
    print(res.status_code, res.json())

def transaction_history(token):
    print_section("Transaction History")
    headers = {"Authorization": f"Bearer {token}"}
    res = requests.get(f"{BASE_URL}/api/payment/history", headers=headers)
    print(res.status_code, res.json())
